...
function run_cubic_logreg_data(
    ...
    sol, numit = AdaProx.adaptive_proxgrad(
        ...
        f = f,
        g = g,
        rule = AdaProx.OurRulePlus(gamma = gam_init, xi = 1, r = 5/6, nu = 1),
        tol = tol / 10,
        maxit = maxit * 10,
        name = nothing,
    )
    xi_values = [1, 1.1, 1.3, 1.5, 2]
    for xi = xi_values
   ...
   for (xi, nu, rp) in parameters
        # r = 1/ (rp *(xi + nu))
        p = 1/rp # rp stands for 1/pi
        r = p/ ((xi + nu))
        # note that pi = r * (xi+nu)
        sol, numit = AdaProx.adaptive_proxgrad(
            zeros(n),
            f = AdaProx.Counting(f),
            g = g,
